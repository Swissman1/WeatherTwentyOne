...
public class DummyWeatherService : IWeatherService
{
    ...
    public DummyWeatherService(HttpClient httpClient)
    {
    ...
    public Task<IEnumerable<Location>> GetLocations(string query)
        ...
        string tempJsonFile = "C:\\Go\\Weather.json";


    public async Task<WeatherForecast> GetWeatherForecast(string zipCode)
    {
        var file = File.ReadAllText(tempJsonFile);
        await Task.Delay(500);
        return JsonConvert.DeserializeObject<WeatherForecast>(file);
    }
}
